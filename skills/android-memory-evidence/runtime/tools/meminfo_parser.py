...
class MemoryCategory:
    ...
    name: str
    pss_total: int = 0           # KB
    pss_clean: int = 0           # KB, older/full dumpsys layout
    shared_dirty: int = 0        # KB, older/full dumpsys layout
    private_dirty: int = 0       # KB
    shared_clean: int = 0        # KB, older/full dumpsys layout
    private_clean: int = 0       # KB
    swap_pss: int = 0            # KB (called Swap Dirty on some older releases)
    rss_total: int = 0           # KB
    ...
    @property
    def swap_dirty(self) -> int:
        """Backward-compatible alias retained for older callers."""
        return self.swap_pss

    @swap_dirty.setter
    def swap_dirty(self, value: int) -> None:
        self.swap_pss = value

    def to_kb_dict(self) -> Dict[str, int]:
        """Return the stable, unit-explicit representation used by report layers."""
        return {
            "pss_total_kb": self.pss_total,
            "pss_clean_kb": self.pss_clean,
            "shared_dirty_kb": self.shared_dirty,
            "private_dirty_kb": self.private_dirty,
            "shared_clean_kb": self.shared_clean,
            "private_clean_kb": self.private_clean,
            "swap_pss_kb": self.swap_pss,
            "rss_total_kb": self.rss_total,
            "heap_size_kb": self.heap_size,
            "heap_alloc_kb": self.heap_alloc,
            "heap_free_kb": self.heap_free,
        }
...
class MeminfoData:
    ...
    dalvik_details: Dict[str, MemoryCategory] = field(default_factory=dict)
...
class MeminfoParser:
    ...
    def _parse_memory_table(self):
        """Parse the main and Dalvik-detail tables across dumpsys layouts."""
        lines = self.content.splitlines()
        separator_index = self._find_main_table_separator(lines)
        if separator_index is not None:
            raw_rows = self._collect_table_rows(lines, separator_index + 1)
            field_names = (
                self._parse_table_fields(lines, separator_index)
                or self._select_table_fields(raw_rows)
            )
            self.data.table_columns = [
                "{}_kb".format(field_name) for field_name in field_names
            ]
            self.data.categories = self._parse_rows(raw_rows, field_names)

        details_index = next(
            (
                index
                for index, line in enumerate(lines)
                if line.strip() == "Dalvik Details"
            ),
            None,
        )
        if details_index is not None:
            raw_details = self._collect_table_rows(lines, details_index + 1)
            field_names = (
                tuple(column[:-3] for column in self.data.table_columns)
                if self.data.table_columns
                else self._select_table_fields(raw_details)
            )
            self.data.dalvik_details = self._parse_rows(raw_details, field_names)

        total_row = self.data.categories.get("TOTAL")
        if total_row:
            self.data.total_pss = total_row.pss_total
            self.data.total_private_dirty = total_row.private_dirty
            self.data.total_rss = total_row.rss_total
            self.data.total_swap = total_row.swap_pss
    ...
    @staticmethod
    def _find_main_table_separator(lines: List[str]):
        """Find the separator belonging to the first process-memory table."""
        for index, line in enumerate(lines):
            if "------" not in line:
                continue
            header = " ".join(lines[max(0, index - 3):index]).lower()
            if "pss" in header and ("private" in header or "shared" in header):
                return index
        return None
    ...
    def _collect_table_rows(lines: List[str], start: int) -> List[str]:
        ...
        for line in lines[start:]:
            ...
            if not stripped:
                break
            if stripped in ("Dalvik Details", "App Summary"):
                break
            if re.search(r"\d", line):
                rows.append(line)
        return rows

    def _select_table_fields(self, rows: List[str]):
        max_values = max(
            (len(self._split_table_row(row)[1]) for row in rows),
            default=0,
        )
        if max_values >= len(self.FULL_TABLE_FIELDS):
            return self.FULL_TABLE_FIELDS
        return self.COMPACT_TABLE_FIELDS

    def _parse_table_fields(self, lines: List[str], separator_index: int):
        """
        Derive the schema from the two aligned dumpsys header rows.

        Android releases and vendor builds expose different subsets of the
        PSS/shared/private/swap/RSS/heap columns, so row width alone is not a
        sufficient schema discriminator.
        """
        ...
        labels = [[] for _ in column_centers]
        for header in lines[max(0, separator_index - 2):separator_index]:
            for token in re.finditer(r"\S+", header):
                token_center = (token.start() + token.end()) / 2
                column_index = min(
                    range(len(column_centers)),
                    key=lambda index: abs(column_centers[index] - token_center),
                )
                labels[column_index].append(token.group(0))

        fields = []
        for parts in labels:
            normalized = re.sub(r"[^a-z0-9]", "", "".join(parts).lower())
            field_name = self.TABLE_HEADER_FIELDS.get(normalized)
            if not field_name:
                return ()
    ...
    def _split_table_row(line: str):
        ...
        if number_index is None or number_index == 0:
            return "", []
        return " ".join(parts[:number_index]), [
            int(value) for value in parts[number_index:] if re.fullmatch(r"-?\d+", value)
        ]

    def _parse_rows(self, rows: List[str], field_names) -> Dict[str, MemoryCategory]:
        parsed = {}
        for row in rows:
            name, values = self._split_table_row(row)
...
def print_meminfo_report(data: MeminfoData):
    ...
    if data.categories:
        print(f"\n--- meminfo 主分类逐行数据 ---")
        print(
            f"{'分类':<16} {'PSS':>9} {'PrivDirty':>10} {'PrivClean':>10} "
            f"{'SwapPss':>9} {'RSS':>9}"
        )
        print("-" * 70)
        for category in data.categories.values():
            print(
                f"{category.name:<16} {category.pss_total:>9} "
                f"{category.private_dirty:>10} {category.private_clean:>10} "
                f"{category.swap_pss:>9} {category.rss_total:>9}"
            )

    if data.dalvik_details:
        print(f"\n--- Dalvik Details ---")
        print(f"{'分类':<16} {'PSS':>9} {'PrivDirty':>10} {'SwapPss':>9} {'RSS':>9}")
        print("-" * 58)
        for category in data.dalvik_details.values():
            print(
                f"{category.name:<16} {category.pss_total:>9} "
                f"{category.private_dirty:>10} {category.swap_pss:>9} "
                f"{category.rss_total:>9}"
            )
  ...
  python3 meminfo_parser.py -f meminfo.txt --json
    ...
    if args.json:
        ...
        output = {
            ...
            'categories': {
                name: category.to_kb_dict()
                for name, category in data.categories.items()
            },
            'dalvik_details': {
                name: category.to_kb_dict()
                for name, category in data.dalvik_details.items()
            },
            'objects': {
